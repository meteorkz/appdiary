function getSheetId() {
  ...
  if (!id) {
    ...
    id = ss.getId(); props.setProperty('SHEET_ID', id);
  }
  return id;
}
...
function doGet(e) {
  ...
  if (a === 'pin')    return jsonRes(togglePin(e.parameter.id, e.parameter.val));
...
function getSheet() {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['id','วันที่เขียนโพส','ข้อมูลโพส','mood','imageUrl','tags','pinned']);
    return sheet;
  }
  const cols = ['mood','imageUrl','tags','pinned'];
...
function getDiaries() {
  try {
    ...
    const h = data[0];
    const gi = function(n){ return h.indexOf(n); };
    return data.slice(1).map(function(r) {
      return { id:String(r[0]), createdAt:String(r[1]), content:String(r[2]),
        mood:String(r[gi('mood')]||''), imageUrl:String(r[gi('imageUrl')]||''),
        tags:String(r[gi('tags')]||''), pinned:String(r[gi('pinned')]||'') };
    }).reverse();
  } catch(e) { return []; }
}
...
function saveDiary(p) {
  try {
    ...
    row[h.indexOf('mood')]=p.mood||''; row[h.indexOf('imageUrl')]=p.imageUrl||'';
    row[h.indexOf('tags')]=p.tags||''; row[h.indexOf('pinned')]='';
    sheet.appendRow(row);
  ...
  try {
    ...
    for (let i=1;i<data.length;i++) {
      if (String(data[i][0])===String(id)) {
        sheet.getRange(i+1,h.indexOf('pinned')+1).setValue(val||'');
        return { success:true };
      }
    }
